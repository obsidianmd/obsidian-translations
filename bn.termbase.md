Thank you for your interest in translating Obsidian. Please add any often used or controversial terms to this file to aid future decisions during documentation or UI translation, thus creating a consistent experience.

Obsidian is constantly under development and as new features are added, so is more text. If you find any term that is not translated yet, feel free to name it. In the language package file (bn.json), we have different text strings. If you have to translate a word, keep that in mind since it can create some inconsistencies.

In doubt, have a look into the translated glossary and feel free to ask Feel free to chat about any improvements, just open new issue or submit pull requests and ping @arg387 plus other active contributors to this language. Thank you.

Please keep the term list in alphabetical order.



|  Original Term                          | Bangla translation              |
| ------------------------------ | --------------------------- |
| Active                         | সক্রিয়                     |
| alias                          | ছদ্মনাম                     |
| Alphabetical                   | বর্ণানুক্রমিক               |
| Assigned                       | নির্ধারিত                   |
| attachment                     | আটাচমেন্ট                   |
| Automatic                      | স্বয়ংক্রিয়                |
| bases                          | বেসেস                       |
| blank                          | ফাঁকা                       |
| blockquote                     | ব্লককোট                     |
| browse                         | ব্রাউজ                      |
| bullet list                    | বুলেট তালিকা                |
| caret                          | ক্যারেট                     |
| collapse                       | বন্ধ                        |
| Community                      | কমিউনিটি                    |
| Configure                      | সাজান                       |
| Copy Share Link                | শেয়ার লিঙ্ককে কপি করুন      |
| Current                        | বর্তমানে                    |
| Custom                         | কাস্টম                      |
| delete                         | মোছা                        |
| drag and drop                  | টেনে এনে ফেলা               |
| edit                           | সম্পাদন                     |
| editing view                   | সম্পাদনা রূপ                |
| edition                        | সংস্করণ                     |
| Editor                         | সম্পাদক                     |
| embed                          | এম্বেড                      |
| excluded                       | বাদ দেয়া                    |
| expand                         | বিস্তার                     |
| export                         | রূপান্তর                    |
| feature                        | ফীচার                       |
| feedback                       | প্রতিক্রিয়া                |
| filter                         | ফিল্টার                     |
| find                           | সন্ধান                      |
| footnote                       | পাদটীকা                     |
| Global command                 | সার্বজনীন কম্যান্ড          |
| hardware acceleration          | হার্ডওয়্যার অ্যাক্সিলারেশন |
| header                         | শিরোনাম                     |
| help                           | সহায়তা                      |
| hidden                         | লুকানো                      |
| highlight                      | হাইলাইট                     |
| horizontal                     | অনুভূমিক                    |
| import                         | ইমপোর্ট                     |
| indent                         | ইন্ডেন্ট                    |
| indentation                    | ইন্ডেন্টেশন                 |
| inline math                    | ইনলাইন গণিত                 |
| insert                         | বসানো                       |
| insider                        | ইনসাইডার                    |
| install                        | ইনস্টল                      |
| installed                      | ইনস্টলড, ইনস্টল করা         |
| internal link                  | ইন্টারনাল লিঙ্ক             |
| internal links                 | ইন্টারনাল লিঙ্কগুলি         |
| keyword                        | কীওয়ার্ড                   |
| Landscape                      | ল্যান্ডস্কেপ                |
| last                           | সর্বশেষ                     |
| latest                         | সর্বশেষ                     |
| layout                         | লেআউট                       |
| legal                          | বৈধ                         |
| location                       | অবস্থান                     |
| Margin                         | মার্জিন                     |
| math block                     | গণিত ব্লক                   |
| maximize                       | সর্বাধিক                    |
| minimize                       | সর্বনিম্ন                   |
| Native                         | নেটিভ                       |
| obsidian                       | অবসিডিয়ান                   |
| Options                        | বিকল্প                      |
| overview                       | সারসংক্ষেপ                  |
| Page                           | পৃষ্ঠা                      |
| pane                           | পেইন                        |
| pane                           | পেইন                        |
| perfix                         | উপসর্গ                      |
| Plugin                         | প্লাগইন                     |
| Quick Action                   | কুইক অ্যাকশন                |
| quote                          | উদ্ধৃতি                     |
| reading view                   | পঠন রূপ                     |
| Recently released              | সম্প্রতি প্রকাশিত           |
| redo(it does opposite of undo) | পুনরায় করুন                |
| reference                      | সূত্র                       |
| Relaunch                       | পুনরায় চালু করুন           |
| reload                         | পুনরায় লোড                 |
| Replace                        | প্রতিস্থাপন                 |
| repository                     | রিপোসিটোরি                  |
| Restart                        | রিস্টার্ট                   |
| restore                        | পুনরুদ্ধার                  |
| Restricted/safe mode           | নিয়ন্ত্রিত মোডে            |
| ribbon                         | রিবন                        |
| save                           | সংরক্ষণ                     |
| search                         | অনুসন্ধান                   |
| Settings                       | সেটিংস                      |
| Star                           | স্টার                       |
| strikethrough                  | স্ট্রাইকথ্রু                |
| style                          | স্টাইল                      |
| Support                        | সমর্থন                      |
| System Trash                   | সিস্টেম ট্র্যাশ             |
| title                          | শিরোনাম                     |
| Toggle                         | টগল                         |
| toolbar                        | টুলবারটি                    |
| translucency                   | স্বচ্ছতা                    |
| Type                           | টাইপ                        |
| Unassigned                     | অনির্ধারিত                  |
| undo                           | পূর্বাবস্থা                 |
| unlinked mention               | আনলিঙ্কড উল্লেখ             |
| up to date                     | আপ টু ডেট                   |
| Update                         | আপডেট                       |
| updated                        | আপডেট                       |
| Validating                     | যাচাই করা হচ্ছে             |
| Version                        | সংস্করণ                     |
| Vertical                       | উল্লম্ব                     |
| visible                        | দেখাও                       |
| writer                         | লেখক                        |





